...
**Domain:** Real-time P2P hexagonal board game (Svelte + Canvas + WebRTC)
...
SvelteKit is the default recommendation for Svelte projects, but it is wrong for this project:
...
Plain Svelte + Vite is the correct choice: a single-page application that builds to static files.

## Installation

```bash
# Scaffold project
npm create vite@latest hex-tic-tac-toe -- --template svelte-ts
cd hex-tic-tac-toe

# Core dependencies
npm install peerjs honeycomb-grid

# Dev dependencies (most come with Vite template)
npm install -D vitest @testing-library/svelte svelte-check
npm install -D prettier prettier-plugin-svelte eslint eslint-plugin-svelte
```
...
Svelte manages state; canvas is an output. The pattern is:
1. Bind `<canvas>` element via `bind:this`
2. In `$effect`, read game state and call imperative canvas draw functions
3. Canvas click events map pixel coordinates -> hex coordinates via honeycomb-grid
...
---
*Stack research for: Hex Tic-Tac-Toe (P2P hexagonal Connect6)*
*Researched: 2026-03-21*
